...
function LoginForm({onLoginAttempted, loggedIn}) {
    const { register, handleSubmit, formState: { errors } } = useForm({mode: "onChange"});
    const onSubmit = data => {
        onLoginAttempted(data.username, data.password);
    }
            ...
            <Form autoComplete="off" onSubmit={handleSubmit(onSubmit)}>
                <Form.Group className="mb-3">
                    ...
                    <input className="form-control"
                        type="text"
                        {...register("username", {
                            minLength: {value: 2, message: "Minimum username length is 2 characters"},
